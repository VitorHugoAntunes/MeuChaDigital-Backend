...
export const createInvitee = async (req: Request, res: Response) => {
  try {
    const { name, phone, email, giftListId, status } = createInviteeSchema.parse(req.body);
    const invitee = await InviteeService.createInvitee({ name, phone, email, giftListId, status });
